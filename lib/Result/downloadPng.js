...
function getLabel(metric, postfix) {
    ...
    if (project)
        return label;
    return ((_a = metric.getLabel) === null || _a === void 0 ? void 0 : _a.call(metric, postfix)) || label + postfix;
}
function drawLegend(pngChart, metrics, textColor, ticker = '') {
    ...
    const textWidth = metrics.reduce((acc, metric) => acc +
        LEGEND_RECT_SIZE +
        LEGEND_RECT_RIGHT_MARGIN +
        pngCtx.measureText(getLabel(metric, postfix)).width, 0) +
...
export function downloadPng(chart, metrics, title) {
    ...
    updateChartState(pngChart, data, categories.joinedCategories, domainModifier, domainGroups, new Set(categories.candles));
...
//# sourceMappingURL=downloadPng.js.map