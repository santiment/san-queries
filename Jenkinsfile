        ...
        if (env.BRANCH_NAME == "main") {
          withCredentials([
            ...
            docker.withRegistry("https://${awsRegistry}", "ecr:eu-central-1:ecr-credentials") {
              sh "docker build --build-arg NODE_GQL_SERVER_URL=http://sanbase.default.svc.cluster.local/graphql --build-arg BACKEND_URL=https://api-stage.santiment.net --build-arg GIT_HEAD=${gitHead} -t ${awsRegistry}/san-queries:stage -t ${awsRegistry}/san-queries:${scmVars.GIT_COMMIT} ."
