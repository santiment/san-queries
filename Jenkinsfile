      ...
      container('docker') {
        ...
        if (env.BRANCH_NAME == "queries-app") {
          withCredentials([
            ...
            docker.withRegistry("https://${awsRegistry}", "ecr:eu-central-1:ecr-credentials") {
              sh "docker build -t ${awsRegistry}/san-queries:stage -t ${awsRegistry}/san-queries:${scmVars.GIT_COMMIT} ."
        ...
        if (env.BRANCH_NAME == "production") {
          withCredentials([
            ...
            def awsRegistry = "${env.aws_account_id}.dkr.ecr.eu-central-1.amazonaws.com"
            docker.withRegistry("https://${awsRegistry}", "ecr:eu-central-1:ecr-credentials") {
              sh "docker build --build-arg GQL_SERVER_URL=http://sanbase.default.svc.cluster.local/graphql --build-arg BACKEND_URL=https://api.santiment.net --build-arg GIT_HEAD=${gitHead} -t ${awsRegistry}/san-queries:production -t ${awsRegistry}/san-queries:${scmVars.GIT_COMMIT} ."
